...
module.exports = function (grunt) {
    ...
    grunt.registerMultiTask('ng_template', 'Make external angular templates inline', function () {
        ...
        function stripTemplates() {
            ...
            let base_file = grunt.file.read(BASE_FILE),
                    ...
                    grunt.file.recurse(filepath, function (abspath, rootdir, subdir, filename) {
                        ...
                        if (filename.indexOf('.html') > -1) {
                            ...
                            grunt.log.ok(configs.PATH.resolve(abspath)
                                ...
                                .replace(/\\/g, '/')
                                .replace((configs.viewsDir) ? new RegExp(`^[\/]{0,1}${configs.viewsDir}\/`) : /^\//, ''));

                            const _filename = configs.PATH.resolve(abspath)
                                    .replace(configs.appDir_path, '')
                                    .replace(/\\/g, '/')
                                    .replace((configs.viewsDir) ? new RegExp(`^[\/]{0,1}${configs.viewsDir}\/`) : /^\//, ''),
        ...
        let new_content = configs.partials.reverse().join(grunt.util.linefeed),
